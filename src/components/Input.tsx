...
const InputTextear = ({ inputSendMessage }: InputTextearProps) => {
    const classNames = {
        start: {
            ...
            textareaClass: ['placeholder:truncate', 'max-h-[190px]', 'duration-200', 'transition-all', 'focus:outline-none', ' flex-1', 'md:text-2xl', 'bg-transparent', 'resize-none'],
            sendClass: ['flex', 'gap-5', 'bg-[#fcfcf9]']
        },
        end: {
            ...
            textareaClass: ['placeholder:truncate', 'col-start-1', 'col-end-4', 'pb-sm', 'overflow-auto', 'outline-none', 'w-full', 'font-sans', 'caret-superDuper', 'resize-none', 'bg-background', 'text-textMain', 'placeholder-textOff', 'max-h-[190px]', 'focus:outline-none', ' flex-1', 'md:text-2xl', 'bg-transparent', 'resize-none'],
            ...
            <div className="xl:w-[1200px] md:w-[800px] sm:w-[650px]  w-[440px] left-[50%] translate-x-[-50%]   z-10   py-sm px-sm md:px-0 fixed flex justify-center pointer-events-none   gap-xl md:bottom-[64px] bottom-[20px]   bg-transparent">
                <div className="pointer-events-auto   bg-offset duration-200transition-all  min-h-10 w-[80%]   md:p-3 p-2 " style={{ borderRadius: inputValue.length < 30 ? '999px' : '20px' }} >
                    <div style={{ borderRadius: inputValue.length < 30 ? '999px' : '20px' }} className={inputValue.length <= 30 ? classNames.start.boxContainerClass.join(' ') : classNames.end.boxContainerClass.join(' ')}>
                        ...
                        <textarea style={{ height: 32, maxHeight: '290px' }} placeholder="Ask follow-up questions" className={inputValue.length <= 30 ? classNames.start.textareaClass.join(' ') : classNames.end.textareaClass.join(' ')} value={inputValue} onChange={handleInputChange} />
                        <div className={inputValue.length <= 30 ? [...classNames.start.sendClass,].join(' ') : classNames.end.sendClass.join(' ')}>
                            <Switch defaultSelected color="default" className="md:block hidden">
