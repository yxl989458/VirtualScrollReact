...
const SourceCard = ({ source }: SourceCardProps) => {
    return (
        <div onClick={() => window.open(source.url)} key={source.id} className="bg-offset opacity-1 ease-in-out  transition-all duration-200 cursor-pointer mb-3 hover:bg-[#e8e8e3]  grid grid-rows-3 grid-cols-1 group basis-96 grow-1  items-stretch w-full relative h-[96px] p-3 gap-4 rounded-md">
